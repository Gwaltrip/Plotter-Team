...
public class Visualizer extends JFrame implements ActionListener, KeyListener,
		MenuListener, MouseListener, MouseWheelListener, MouseMotionListener,
		FocusListener {
	...
	public Visualizer() {
		...
		setLocation(20, 20);
		setTitle(VERSION);
		setSize(LEFTBORDER + WIDTH + RIGHTBORDER, UPBORDER + HEIGHT
				+ BUTTOMBORDER);
	...
	private void buildBottomPanel() {
		...
		bottom.setBounds(0, UPBORDER + HEIGHT,
				LEFTBORDER + WIDTH + RIGHTBORDER, BUTTOMBORDER);
		add(bottom);
	...
	public void loadProperties() {
		...
		try {
			property.load(new FileInputStream("mathgraphics.properties"));
	...
	private void selectColors() {
		...
		Colorpanel cp = new Colorpanel(property);
		setColors(property);
	...
	private void setInitColors(Properties p2) {
		...
		if (p2.getProperty("BACKGROUND_COLOR") != null) {
			BACKGROUND_COLOR = Colorpanel.buildColor(p2
					.getProperty("BACKGROUND_COLOR"));
		} else {
			p2.setProperty("BACKGROUND_COLOR",
					Colorpanel.decomposeColor(BACKGROUND_COLOR));
		}
		...
		if (p2.getProperty("PANEL_COLOR") != null) {
			...
			p2.setProperty("PANEL_COLOR",
					Colorpanel.decomposeColor(PANEL_COLOR));
		}
		...
		if (p2.getProperty("LINE_2_COLOR") != null) {
			LINE_2_COLOR = Colorpanel
					.buildColor(p2.getProperty("LINE_2_COLOR"));

		} else {
			p2.setProperty("LINE_2_COLOR",
					Colorpanel.decomposeColor(LINE_2_COLOR));
		}
		...
		if (p2.getProperty("LINE_3D_COLOR") != null) {
			LINE_3D_COLOR = Colorpanel.buildColor(p2
					.getProperty("LINE_3D_COLOR"));
		} else {
			p2.setProperty("LINE_3D_COLOR",
					Colorpanel.decomposeColor(LINE_3D_COLOR));
		}
	...
	public void mouseClicked(MouseEvent arg0) {
		 ...
		 * int BORDER_LIMIT=50;
		 * 
		 * Point point = arg0.getPoint(); if(point.y<BORDER_LIMIT) up(-1);
	...
	public void mouseMoved(MouseEvent arg0) {
		...
		if (VISUALIZATION_STATE == CARTESIAN2D_STATE
				|| VISUALIZATION_STATE == POLAR2D_STATE)
			screenPoint.setText(calc.invertX((int) p.getX()) + " ; "
					+ calc.invertY((int) p.getY(), HEIGHT));
		else
			screenPoint.setText("");
	}
